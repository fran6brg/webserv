...
if len(tests_to_run) == 0 or i in tests_to_run:
	r = requests.post('http://localhost:8080/text', payload, headers=headers_adds)
	printResponse(r, i)
...
if len(tests_to_run) == 0 or i in tests_to_run:
	r = requests.post('http://localhost:8080/TEST4', payload, headers=headers_adds)
	printResponse(r, i)
...
# ---
i += 1
print (bcolors.OKBLUE + "\n" + str(i) + ". POST http://localhost:8080/test/bonjour sans cgi \n" + bcolors.ENDC)
body = "bonjour"
if len(tests_to_run) == 0 or i in tests_to_run:
	r = requests.post('http://localhost:8080/bj', data=body)
	printResponse(r, i)
...
body = "x" * 1000
if len(tests_to_run) == 0 or i in tests_to_run:
	r = requests.post('http://localhost:8080/test.cgi', data=body)
	printResponse(r, i)
